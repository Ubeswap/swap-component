...
describe('multicall updater', () => {
    ...
    describe('#outdatedListeningKeys', () => {
        ...
        it('returns only keys not being fetched', () => {
            expect(outdatedListeningKeys({
                [1]: { abc: { data: '0x', blockNumber: 2 }, def: { fetchingBlockNumber: 2 } },
            }, { abc: 1, def: 1 }, 1, 2)).toEqual([]);
        });
        it('returns keys being fetched for old blocks', () => {
            expect(outdatedListeningKeys({ [1]: { abc: { data: '0x', blockNumber: 2 }, def: { fetchingBlockNumber: 1 } } }, { abc: 1, def: 1 }, 1, 2)).toEqual(['def']);
        });
        it('respects blocks per fetch', () => {
            expect(outdatedListeningKeys({ [1]: { abc: { data: '0x', blockNumber: 2 }, def: { data: '0x', fetchingBlockNumber: 1 } } }, { abc: 2, def: 2 }, 1, 3)).toEqual(['def']);
        });
    });
});
