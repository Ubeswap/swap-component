...
export default function ConfirmSwapModal({ trade, originalTrade, onAcceptChanges, allowedSlippage, onConfirm, onDismiss, recipient, swapErrorMessage, isOpen, attemptingTxn, txHash, }) {
    ...
    const modalHeader = useCallback(() => {
        return trade ? (_jsx(SwapModalHeader, { trade: trade, allowedSlippage: allowedSlippage, recipient: recipient, showAcceptChanges: showAcceptChanges, onAcceptChanges: onAcceptChanges }, void 0)) : null;
    ...
    const confirmationContent = useCallback(() => swapErrorMessage ? (_jsx(TransactionErrorContent, { onDismiss: onDismiss, message: swapErrorMessage }, void 0)) : (_jsx(ConfirmationModalContent, { title: `Confirm ${label}`, onDismiss: onDismiss, topContent: modalHeader, bottomContent: modalBottom }, void 0)), [onDismiss, modalBottom, modalHeader, swapErrorMessage, label]);
