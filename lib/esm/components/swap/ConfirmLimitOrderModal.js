...
export default function ConfirmLimitOrderModal({ trade, originalTrade, onAcceptChanges, allowedSlippage, onConfirm, onDismiss, recipient, swapErrorMessage, isOpen, attemptingTxn, txHash, }) {
    ...
    const modalHeader = useCallback(() => {
        return trade ? (_jsx(LimitOrderModalHeader, { trade: trade, allowedSlippage: allowedSlippage, recipient: recipient, showAcceptChanges: showAcceptChanges, onAcceptChanges: onAcceptChanges })) : null;
    ...
    const confirmationContent = useCallback(() => swapErrorMessage ? (_jsx(TransactionErrorContent, { onDismiss: onDismiss, message: swapErrorMessage })) : (_jsx(ConfirmationModalContent, { title: `Confirm ${t('limitOrder').toLowerCase()}`, onDismiss: onDismiss, topContent: modalHeader, bottomContent: modalBottom })), [swapErrorMessage, onDismiss, t, modalHeader, modalBottom]);
