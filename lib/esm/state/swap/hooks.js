...
export function useDerivedSwapInfo() {
    ...
    const bestTradeExactIn = useUbeswapTradeExactIn(isExactIn ? parsedAmount : undefined, outputCurrency !== null && outputCurrency !== void 0 ? outputCurrency : undefined);
    ...
    if (balanceIn && amountIn && balanceIn.lessThan(amountIn)) {
        if (amountIn.currency.address === cUSD[network.chainId].address ||
            amountIn.currency.address === CELO[network.chainId].address ||
            amountIn.currency.address === cEUR[network.chainId].address) {
            showRamp = true;
        }
...
function parseCurrencyFromURLParameter(urlParam, chainId) {
    ...
    if ([ChainId.EthereumMainnet, ChainId.Kovan].includes(chainId)) {
        return '';
    }
